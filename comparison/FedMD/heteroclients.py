...
class Client_init:
    ...
    """
    def __init__(self, args, dataset_name, train_ratio, public_num, init):
        ...
        if self.dataset_name == 'Ku':
            ...
            # read data
            self.fault_dict = np.load('/data/wjy/Promising_idea/data/Semantic/raw_data.npy', allow_pickle=True).item()
        
        if self.dataset_name == 'TE':
            self.class_number = 15
            self.train_ratio = train_ratio
            
            self.path = '/data/wjy/Promising_idea/data/extend_TE/'
            fault1  = np.load(self.path + 'fault_1.npy')[:6]
            fault2  = np.load(self.path + 'fault_2.npy')[:6]
            fault3  = np.load(self.path + 'fault_3.npy')[:6]
            fault4  = np.load(self.path + 'fault_4.npy')[:6]
            fault5  = np.load(self.path + 'fault_5.npy')[:6]
            fault6  = np.load(self.path + 'fault_6.npy')[:6]
            fault7  = np.load(self.path + 'fault_7.npy')[:6]
            fault8  = np.load(self.path + 'fault_8.npy')[:6]
            fault9  = np.load(self.path + 'fault_9.npy')[:6]
            fault10 = np.load(self.path + 'fault_10.npy')[:6]
            fault11 = np.load(self.path + 'fault_11.npy')[:6]
            fault12 = np.load(self.path + 'fault_12.npy')[:6]
            fault13 = np.load(self.path + 'fault_13.npy')[:6]
            fault14 = np.load(self.path + 'fault_14.npy')[:6]
            fault15 = np.load(self.path + 'fault_15.npy')[:6]
            
            self.fault_dict = {0:fault1[:5], 1:fault2[:5], 2:fault3[:5], 3:fault4[:5], 4:fault5[:5],
                               5:fault6[:5], 6:fault7[:5], 7:fault8[:5], 8:fault9[:5], 9:fault10[:5],
                               10:fault11[:5], 11:fault12[:5], 12:fault13[:5], 13:fault14[:5], 14:fault15[:5]}
            self.public_fault_dict = {0:fault1[5:], 1:fault2[5:], 2:fault3[5:], 3:fault4[5:], 4:fault5[5:],
                                      5:fault6[5:], 6:fault7[5:], 7:fault8[5:], 8:fault9[5:], 9:fault10[5:],
                                      10:fault11[5:], 11:fault12[5:], 12:fault13[5:], 13:fault14[5:], 14:fault15[5:]}
        ...
        if init:
            if args.resplit:
                ...
                public, public_valid = self.concat_public(public_num)
            else:
                ...
                public = np.load(os.path.join(self.path, "public/public.npy"), allow_pickle=True).item()
                public_valid = np.load(os.path.join(self.path, "public/public_valid.npy"), allow_pickle=True).item()
        else:
            print("------------ Load pre-splited data! ------------")
            train_data = np.load(os.path.join(self.path, "train/train.npy"), allow_pickle=True).item()
            test_data = np.load(os.path.join(self.path, "test/test.npy"), allow_pickle=True).item()
            public = np.load(os.path.join(self.path, "public.npy"), allow_pickle=True).item()
            public_valid = np.load(os.path.join(self.path, "public/public_valid.npy"), allow_pickle=True).item()
        ...
        self.public = public
        self.public_valid = public_valid
    ...
    def concat_public(self, public_num):
        for key, value in self.public_fault_dict.items():
            value = np.delete(value, np.arange(20), 1)
            value = np.delete(value, np.arange(3), 2)
            self.public_fault_dict[key] = value.reshape(-1, 52)

        data_path=f'{self.path}public'
        if not os.path.exists(data_path):
            os.makedirs(data_path)
        ...
        public_label = []
        for key, value in self.public_fault_dict.items():
            value = value[:public_num, :]
            public_data.append(value)
            public_label.append(np.repeat(key, value.shape[0]))
        public_data = np.concatenate(public_data, axis=0)
        public_label = np.concatenate(public_label)

        public['data'] = public_data
        public['label'] = public_label

        data_path=os.path.join(self.path, "public/public.npy")
        with open(data_path, 'wb') as outfile:
            print(f"Dumping public data => {data_path}")
            np.save(outfile, public)
        ...
        for key, value in self.public_fault_dict.items():
            value = value[public_num:, :]
            public_validdata.append(value)
            public_validlabel.append(np.repeat(key, value.shape[0]))
        public_validdata = np.concatenate(public_validdata, axis=0)
        public_validlabel = np.concatenate(public_validlabel)

        public_valid['data'] = public_validdata
        public_valid['label'] = public_validlabel

        data_path=os.path.join(self.path, "public/public_valid.npy")
        with open(data_path, 'wb') as outfile:
            print(f"Dumping public data => {data_path}")
            np.save(outfile, public_valid)

        return public, public_valid
